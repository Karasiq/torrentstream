...
import com.karasiq.ttorrent.common.protocol.TrackerMessage;
import com.karasiq.ttorrent.bcodec.BDecoder;
...
public class HTTPAnnounceRequestMessage extends HTTPTrackerMessage
	...
	private final com.karasiq.ttorrent.common.Peer peer;
	...
	private HTTPAnnounceRequestMessage(ByteBuffer data,
									   byte[] infoHash, com.karasiq.ttorrent.common.Peer peer, long uploaded, long downloaded,
	...
	public String getHexInfoHash() {
		return com.karasiq.ttorrent.common.Torrent.byteArrayToHexString(this.infoHash);
	}
	...
	public URL buildAnnounceURL(URL trackerAnnounceURL)
		...
		url.append(base.contains("?") ? "&" : "?")
			...
			.append(URLEncoder.encode(
				new String(this.getInfoHash(), com.karasiq.ttorrent.common.Torrent.BYTE_ENCODING),
				com.karasiq.ttorrent.common.Torrent.BYTE_ENCODING))
			.append("&peer_id=")
			.append(URLEncoder.encode(
				new String(this.getPeerId(), com.karasiq.ttorrent.common.Torrent.BYTE_ENCODING),
				com.karasiq.ttorrent.common.Torrent.BYTE_ENCODING))
	...
	public static HTTPAnnounceRequestMessage parse(ByteBuffer data)
		...
		com.karasiq.ttorrent.bcodec.BEValue decoded = BDecoder.bdecode(data);
		if (decoded == null) {
		...
		if (!params.containsKey("info_hash")) {
			throw new TrackerMessage.MessageValidationException(
				TrackerMessage.ErrorMessage.FailureReason.MISSING_HASH.getMessage());
		}

		if (!params.containsKey("peer_id")) {
			throw new TrackerMessage.MessageValidationException(
				TrackerMessage.ErrorMessage.FailureReason.MISSING_PEER_ID.getMessage());
		}

		if (!params.containsKey("port")) {
			throw new TrackerMessage.MessageValidationException(
				TrackerMessage.ErrorMessage.FailureReason.MISSING_PORT.getMessage());
		}
		...
		try {
			...
			int numWant = TrackerMessage.AnnounceRequestMessage.DEFAULT_NUM_WANT;
			...
			if (params.containsKey("ip")) {
				ip = params.get("ip").getString(com.karasiq.ttorrent.common.Torrent.BYTE_ENCODING);
			}

			RequestEvent event = RequestEvent.NONE;
			if (params.containsKey("event")) {
				event = RequestEvent.getByName(params.get("event")
					.getString(com.karasiq.ttorrent.common.Torrent.BYTE_ENCODING));
			}

			return new HTTPAnnounceRequestMessage(data, infoHash,
				new com.karasiq.ttorrent.common.Peer(ip, port, ByteBuffer.wrap(peerId)),
	...
	public static HTTPAnnounceRequestMessage craft(byte[] infoHash,
		...
		Map<String, com.karasiq.ttorrent.bcodec.BEValue> params = new HashMap<String, com.karasiq.ttorrent.bcodec.BEValue>();
		params.put("info_hash", new com.karasiq.ttorrent.bcodec.BEValue(infoHash));
		params.put("peer_id", new com.karasiq.ttorrent.bcodec.BEValue(peerId));
		params.put("port", new com.karasiq.ttorrent.bcodec.BEValue(port));
		params.put("uploaded", new com.karasiq.ttorrent.bcodec.BEValue(uploaded));
		params.put("downloaded", new com.karasiq.ttorrent.bcodec.BEValue(downloaded));
		params.put("left", new com.karasiq.ttorrent.bcodec.BEValue(left));
		params.put("compact", new com.karasiq.ttorrent.bcodec.BEValue(compact ? 1 : 0));
		params.put("no_peer_id", new com.karasiq.ttorrent.bcodec.BEValue(noPeerId ? 1 : 0));

		if (event != null) {
			params.put("event",
				new com.karasiq.ttorrent.bcodec.BEValue(event.getEventName(), com.karasiq.ttorrent.common.Torrent.BYTE_ENCODING));
		}

		if (ip != null) {
			params.put("ip",
				new com.karasiq.ttorrent.bcodec.BEValue(ip, com.karasiq.ttorrent.common.Torrent.BYTE_ENCODING));
		}

		if (numWant != TrackerMessage.AnnounceRequestMessage.DEFAULT_NUM_WANT) {
			params.put("numwant", new com.karasiq.ttorrent.bcodec.BEValue(numWant));
		}

		return new HTTPAnnounceRequestMessage(
			com.karasiq.ttorrent.bcodec.BEncoder.bencode(params),
			infoHash, new com.karasiq.ttorrent.common.Peer(ip, port, ByteBuffer.wrap(peerId)),
