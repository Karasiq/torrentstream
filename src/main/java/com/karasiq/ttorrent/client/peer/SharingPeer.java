 ...
 */
package com.karasiq.ttorrent.client.peer;

import com.karasiq.ttorrent.common.Peer;
import com.karasiq.ttorrent.client.Piece;
import com.karasiq.ttorrent.client.SharedTorrent;
...
public class SharingPeer extends Peer implements MessageListener {
	...
	private BlockingQueue<com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage> requests;
	private volatile boolean downloading;

	private PeerExchange exchange;
	private com.karasiq.ttorrent.client.peer.Rate download;
	private com.karasiq.ttorrent.client.peer.Rate upload;
	...
	public com.karasiq.ttorrent.client.peer.Rate getDLRate() {
		return this.download;
	}

	public com.karasiq.ttorrent.client.peer.Rate getULRate() {
		return this.upload;
	}
	...
	public void choke() {
		if (!this.choking) {
			...
			this.send(com.karasiq.ttorrent.common.protocol.PeerMessage.ChokeMessage.craft());
			this.choking = true;
		}
	}
	...
	public void unchoke() {
		if (this.choking) {
			...
			this.send(com.karasiq.ttorrent.common.protocol.PeerMessage.UnchokeMessage.craft());
			this.choking = false;
		}
	}
	...
	public void interesting() {
		if (!this.interesting) {
			...
			this.send(com.karasiq.ttorrent.common.protocol.PeerMessage.InterestedMessage.craft());
	...
	public void notInteresting() {
		if (this.interesting) {
			...
			this.send(com.karasiq.ttorrent.common.protocol.PeerMessage.NotInterestedMessage.craft());
			this.interesting = false;
		}
	}
	...
	public synchronized void bind(SocketChannel channel) throws SocketException {
		...
		this.download = new com.karasiq.ttorrent.client.peer.Rate();
		this.download.reset();

		this.upload = new com.karasiq.ttorrent.client.peer.Rate();
		this.upload.reset();
	}
	 ...
	 * exchange.
	 */
	public void send(com.karasiq.ttorrent.common.protocol.PeerMessage message) throws IllegalStateException {
		if (this.isConnected()) {
	...
	public synchronized void downloadPiece(Piece piece)
		...
		this.requests = new LinkedBlockingQueue<com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage>(
	...
	private void requestNextBlocks() {
		synchronized (this.requestsLock) {
			...
			while (this.requests.remainingCapacity() > 0 &&
				...
				com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage request = com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage
					.craft(
						...
						Math.min(
							(int)(this.requestedPiece.size() -
								this.lastRequestedOffset),
							com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage.DEFAULT_REQUEST_SIZE));
	 ...
	 */
	private void removeBlockRequest(com.karasiq.ttorrent.common.protocol.PeerMessage.PieceMessage message) {
		synchronized (this.requestsLock) {
			if (this.requests == null) {
				return;
			}

			for (com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage request : this.requests) {
	 ...
	 * </p>
	 */
	public Set<com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage> cancelPendingRequests() {
		synchronized (this.requestsLock) {
			Set<com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage> requests =
				new HashSet<com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage>();

			if (this.requests != null) {
				for (com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage request : this.requests) {
					this.send(com.karasiq.ttorrent.common.protocol.PeerMessage.CancelMessage.craft(request.getPiece(),
	 ...
	 */
	@Override
	public synchronized void handleMessage(com.karasiq.ttorrent.common.protocol.PeerMessage msg) {
		switch (msg.getType()) {
			case KEEP_ALIVE:
			...
			case HAVE:
				...
				com.karasiq.ttorrent.common.protocol.PeerMessage.HaveMessage have = (com.karasiq.ttorrent.common.protocol.PeerMessage.HaveMessage)msg;
			...
			case BITFIELD:
				...
				com.karasiq.ttorrent.common.protocol.PeerMessage.BitfieldMessage bitfield =
					(com.karasiq.ttorrent.common.protocol.PeerMessage.BitfieldMessage)msg;
				...
				break;
			case REQUEST:
				com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage request =
					(com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage)msg;
				...
				if (request.getLength() >
						com.karasiq.ttorrent.common.protocol.PeerMessage.RequestMessage.MAX_REQUEST_SIZE) {
				...
				try {
					ByteBuffer block = rp.read(request.getOffset(),
									request.getLength());
					this.send(com.karasiq.ttorrent.common.protocol.PeerMessage.PieceMessage.craft(request.getPiece(),
			...
			case PIECE:
				...
				// greedy?
				com.karasiq.ttorrent.common.protocol.PeerMessage.PieceMessage piece = (com.karasiq.ttorrent.common.protocol.PeerMessage.PieceMessage)msg;
	 ...
	 *
	 * @author mpetazzoni
	 * @see com.karasiq.ttorrent.client.peer.Rate.RateComparator
	 */
	public static class DLRateComparator
		...
		public int compare(SharingPeer a, SharingPeer b) {
			return com.karasiq.ttorrent.client.peer.Rate.RATE_COMPARATOR.compare(a.getDLRate(), b.getDLRate());
		}
	}
	...
	public static class ULRateComparator
		...
		@Override
		public int compare(SharingPeer a, SharingPeer b) {
			return com.karasiq.ttorrent.client.peer.Rate.RATE_COMPARATOR.compare(a.getULRate(), b.getULRate());
		}
	}
