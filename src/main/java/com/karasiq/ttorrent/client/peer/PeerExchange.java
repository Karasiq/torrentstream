...
class PeerExchange {
	...
	private Set<com.karasiq.ttorrent.client.peer.MessageListener> listeners;

	private IncomingThread in;
	private OutgoingThread out;
	private BlockingQueue<com.karasiq.ttorrent.common.protocol.PeerMessage> sendQueue;
	...
	public PeerExchange(SharingPeer peer, SharedTorrent torrent,
		...
		this.listeners = new HashSet<com.karasiq.ttorrent.client.peer.MessageListener>();
		this.sendQueue = new LinkedBlockingQueue<com.karasiq.ttorrent.common.protocol.PeerMessage>();
		...
		if (pieces.cardinality() > 0) {
			this.send(com.karasiq.ttorrent.common.protocol.PeerMessage.BitfieldMessage.craft(pieces));
		}
	}
	 ...
	 */
	public void register(com.karasiq.ttorrent.client.peer.MessageListener listener) {
	 ...
	 */
	public void send(com.karasiq.ttorrent.common.protocol.PeerMessage message) {
		try {
	...
	private abstract class RateLimitThread extends Thread {
		...
		protected final com.karasiq.ttorrent.client.peer.Rate rate = new com.karasiq.ttorrent.client.peer.Rate();
		 ...
		 */
		protected void rateLimit(double maxRate, long messageSize, com.karasiq.ttorrent.common.protocol.PeerMessage message) {
			if (message.getType() != com.karasiq.ttorrent.common.protocol.PeerMessage.Type.PIECE || maxRate <= 0) {
				return;
			}
	...
	private class IncomingThread extends RateLimitThread {
		...
		public void run() {
			...
			try {
				...
				while (!stop) {
					buffer.rewind();
					buffer.limit(com.karasiq.ttorrent.common.protocol.PeerMessage.MESSAGE_LENGTH_FIELD_SIZE);
					...
					buffer.limit(com.karasiq.ttorrent.common.protocol.PeerMessage.MESSAGE_LENGTH_FIELD_SIZE + pstrlen);
					...
					try {
						com.karasiq.ttorrent.common.protocol.PeerMessage message = com.karasiq.ttorrent.common.protocol.PeerMessage.parse(buffer, torrent);
						...
						for (com.karasiq.ttorrent.client.peer.MessageListener listener : listeners)
			...
			try {
				...
				while (!stop || (stop && sendQueue.size() > 0)) {
					try {
						...
						com.karasiq.ttorrent.common.protocol.PeerMessage message = sendQueue.poll(
						...
						if (message == null) {
							...
							message = com.karasiq.ttorrent.common.protocol.PeerMessage.KeepAliveMessage.craft();
						}
