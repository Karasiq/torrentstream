 ...
 * {@link #announce(com.karasiq.ttorrent.tracker.TrackedTorrent torrent)}</code> method.
 * </p>
 *
 * @author mpetazzoni
 */
public class Tracker {
	...
	private final ConcurrentMap<String, com.karasiq.ttorrent.tracker.TrackedTorrent> torrents;
	...
	public Tracker(InetSocketAddress address, String version)
		...
		this.torrents = new ConcurrentHashMap<String, com.karasiq.ttorrent.tracker.TrackedTorrent>();
	 ...
	 */
	public Collection<com.karasiq.ttorrent.tracker.TrackedTorrent> getTrackedTorrents() {
	 ...
	 */
	public synchronized com.karasiq.ttorrent.tracker.TrackedTorrent announce(com.karasiq.ttorrent.tracker.TrackedTorrent torrent) {
		com.karasiq.ttorrent.tracker.TrackedTorrent existing = this.torrents.get(torrent.getHexInfoHash());
	 ...
	 */
	public synchronized void remove(com.karasiq.ttorrent.common.Torrent torrent) {
		if (torrent == null) {
			return;
		}
	 ...
	 */
	public synchronized void remove(com.karasiq.ttorrent.common.Torrent torrent, long delay) {
		if (torrent == null) {
			return;
		}
	...
	private static class TorrentRemoveTimer extends TimerTask {
		...
		private com.karasiq.ttorrent.common.Torrent torrent;

		TorrentRemoveTimer(Tracker tracker, com.karasiq.ttorrent.common.Torrent torrent) {
	...
	private class PeerCollectorThread extends Thread {
		...
		public void run() {
			...
			while (!stop) {
				for (com.karasiq.ttorrent.tracker.TrackedTorrent torrent : torrents.values()) {
