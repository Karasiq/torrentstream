 ...
 */
package com.karasiq.ttorrent.tracker;
...
public class TrackerService implements Container {
	...
	private void process(Request request, Response response,
		 ...
		 */
		com.karasiq.ttorrent.common.protocol.http.HTTPAnnounceRequestMessage announceRequest = null;
		try {
		...
		TrackerMessage.AnnounceRequestMessage.RequestEvent event = announceRequest.getEvent();
		...
		if ((event == null ||
				TrackerMessage.AnnounceRequestMessage.RequestEvent.NONE.equals(event)) &&
		...
		if (event != null && torrent.getPeer(peerId) == null &&
			!TrackerMessage.AnnounceRequestMessage.RequestEvent.STARTED.equals(event)) {
			this.serveError(response, body, Status.BAD_REQUEST,
		...
		com.karasiq.ttorrent.common.protocol.http.HTTPAnnounceResponseMessage announceResponse = null;
		try {
			announceResponse = com.karasiq.ttorrent.common.protocol.http.HTTPAnnounceResponseMessage.craft(
	 ...
	 * @return The {@link TrackerMessage.AnnounceRequestMessage} representing the client's
	 * announce request.
	 */
	private com.karasiq.ttorrent.common.protocol.http.HTTPAnnounceRequestMessage parseQuery(Request request)
		...
		if (params.get("ip") == null) {
			params.put("ip", new com.karasiq.ttorrent.bcodec.BEValue(
				request.getClientAddress().getAddress().getHostAddress(),
				TrackedTorrent.BYTE_ENCODING));
		}


		return com.karasiq.ttorrent.common.protocol.http.HTTPAnnounceRequestMessage.parse(com.karasiq.ttorrent.bcodec.BEncoder.bencode(params));
	}

	private void recordParam(Map<String, com.karasiq.ttorrent.bcodec.BEValue> params, String key,
							 String value) {
		try {
			...
			for (String f : NUMERIC_REQUEST_FIELDS) {
				if (f.equals(key)) {
					params.put(key, new com.karasiq.ttorrent.bcodec.BEValue(Long.valueOf(value)));
					return;
				}
			}

			params.put(key, new com.karasiq.ttorrent.bcodec.BEValue(value, TrackedTorrent.BYTE_ENCODING));
	...
	/**
	 * Write a {@link com.karasiq.ttorrent.common.protocol.http.HTTPTrackerErrorMessage} to the response with the given
	 * HTTP status code.
	 *
	...
	private void serveError(Response response, OutputStream body,
		Status status, com.karasiq.ttorrent.common.protocol.http.HTTPTrackerErrorMessage error) throws IOException {
	...
	private void serveError(Response response, OutputStream body,
		...
		try {
			this.serveError(response, body, status,
				com.karasiq.ttorrent.common.protocol.http.HTTPTrackerErrorMessage.craft(error));
		} catch (TrackerMessage.MessageValidationException mve) {
